...
# This module is only for generating team comp recommendations based on prexisiting 
# units, team size, and the unit pool as a constraints.

def conditional_prob_unit_check ( conditional_units, target_unit , df:pd.DataFrame ):
    # Get the sampled probabilistic distance from target unit given the conditional unit's exisitance
...
def build_naive_conditional_comp ( team_size, conditional_units, unit_pool, pre_computed_conditional_prob:pd.DataFrame ):
     # Get the average probabilistic distance from target unit given the conditional unit's exisitance
    conditional_units = [
        unit if unit.startswith(UNIT_PREFIX) else f"unit_TFT14_{unit}"
        for unit in conditional_units
    ]
